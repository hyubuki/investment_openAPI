...
현재 문서는 B2C 개인 투자자 OpenAPI를 위한 `Draft v0.2`다. 거래소 규정과 내부 OMS 계약이 확인되면 ADR과 OpenAPI 문서로 확정한다.
...
| 업무 모듈 | 핵심 책임 |
|---|---|
| 사용자 | 개인 투자자의 가입, 인증, Token 및 Session 관리 |
...
- 시장 모듈은 상품과 시장 상태를 변경하며 사용자 주문 상태를 변경하지 않는다.
